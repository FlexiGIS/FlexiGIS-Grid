...
def skeletonize(roads, buffer_length=60,
    ...
    streets_buffered_merged = select_biggest_polygon_from_multipolygon(
        streets_buffered_merged)
    streets_buffered_merged = streets_buffered_merged.buffer(-dissolve_length)
    streets_buffered_merged = select_biggest_polygon_from_multipolygon(
        streets_buffered_merged)
    streets_buffered_merged_simplified = streets_buffered_merged.simplify(
        simplify_length)

    # then calculate skeleton (expensive but necessary)
    street_graphs = Skeletron.polygon_skeleton_graphs(
        streets_buffered_merged_simplified, psg_length)
    ...
    # merge the result
    street_lines_merged_intersect = street_lines_merged.intersection(
        whole_city)
    street_lines_merged_intersect_merged = shapely.ops.linemerge(
        street_lines_merged_intersect)
